...
export default function Categories({ posts }) {
  ...
  const [order, setOrder] = useState(
    typeof window !== "undefined" &&
      localStorage.getItem("categoryOrdered") === "true"
      ? true
      : false
  );
  if (!order) {
    ...
    uniqueCategories.sort((a, b) => postCount[b] - postCount[a]);
  }
  typeof window !== "undefined" &&
    localStorage.setItem("categoryOrdered", order);
                  ...
                  <div key={i} className="col-md-4 col-6">
                    ...
                    >
                      <div className="flex-grow-1">
                        {category}
                        <small className="ms-1">({postCount[category]})</small>
                      </div>
